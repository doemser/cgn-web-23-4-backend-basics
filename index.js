...
export const server = createServer((request, response) => {
  if (request.url === "/bad-jokes") {
    response.statusCode = 400;
    response.end(JSON.stringify(badJokes));
  } else if (request.url === "/brokkoli") {
    response.statusCode = 200;
    response.end("Brokkoli");
  } else if (request.url === "/not-brokkoli") {
    response.statusCode = 200;
    response.end("Nicht Brokkoli");
  } else {
